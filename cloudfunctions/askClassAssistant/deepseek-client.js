...
const sendHttpsRequest = ({ url, headers, body, timeoutMs, shouldCancel }) => new Promise((resolve, reject) => {
  let settled = false;
  let controlTimer;
    ...
    controlTimer = setInterval(async () => {
      try {
        if (await shouldCancel()) {
          request.destroy(createError("AI request cancelled", { errorType: "cancelled", stage: "deepseek_request" }));
        }
      } catch (error) {
...
const requestDeepSeek = async (config, messages, options = {}) => {
  ...
  try {
    response = await requestImpl({
      ...
      body,
      timeoutMs: Math.max(1000, Number(options.timeoutMs) || 45000),
