...
这个项目适合班级、社团、学习小组或课程团队使用：管理员负责发布和维护事项，成员完成身份认证后即可查看、收藏、订阅提醒，并可按需使用基于学生手册的班级助手；持公共访问码的访客只能使用同一个班级助手；管理员也可以使用 AI 快速发布，把一句话通知整理成可确认、可修改的事项草稿。
...
| 角色 | 可以做什么 |
| --- | --- |
| 未认证用户 | 选择班级成员认证或使用公共访问码进入 AI 助手 |
| 已认证成员 | 查看事项、收藏事项、订阅提醒、提交意见反馈；启用后可使用班级助手 |
| AI 助手访客 | 通过公共访问码使用班级助手，不能访问班级事项和管理功能 |
...
班级助手供已认证成员和合法访客使用，二者共用同一个页面、学生手册知识库、检索与回答逻辑。每个 OpenID 每分钟最多提问 3 次；普通成员、管理员和访客每天最多 20 次，超级管理员每天最多 50 次。检索不到匹配手册内容或命中固定补充说明时不会调用 AI，也不会消耗每日次数。
...
- `verifyGuestAccess`：校验公共访问码并建立当前 OpenID 的访客身份。
- `clearGuestAccess`：清除当前 OpenID 的访客身份，不影响正式成员记录。
- `createNotice` / `updateNotice` / `deleteNotice`：管理事项。
- `updateNoticePin`：更新置顶状态。
- `parseNoticeWithAI`：将管理员输入的一句话解析为事项草稿。
- `askClassAssistant`：供正式成员和访客检索同一份学生手册并生成回答，也负责处理带身份校验的停止信号。
