...
班级盒子会处理班级成员身份、通知内容、收藏记录、订阅授权、管理员权限和学生手册问答。自行部署时，建议把它当作一个真实班级系统来配置，而不只是一个演示小程序。
...
- `data/*.pdf`
- `data/handbook-*.txt`
- 真实版本的 `data/handbook_versions_*_import.json` 和 `data/handbook_chunks_*_import.json`
...
班级助手应在检索和 AI 调用前检测学生问题。AI 只能依据检索到的手册片段回答；非“未找到明确规定”的手册回答必须包含依据。固定补充说明必须明确标注不属于学生手册，并说明适用边界。停止操作通过带身份校验的取消信号终止上游请求，不能只停止前端动画。
...
- 仓库中没有真实学生手册 PDF、文本或切片数据，只保留 example 数据。
- 仓库中没有 `cloud://` 文件地址或云存储 fileID。
- 数据库权限已经按最小权限配置，并用普通用户账号测试过。
- `askClassAssistant` 没有暴露未认证的公网 HTTP 路由。
